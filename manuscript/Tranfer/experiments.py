...
def run_experiment(model, model_kwargs=None, train_loader=None, test_loader=None, device='cpu', epochs=10, workflow='default', exp_name='experiment', collapse_range=None, data_shape=(1, 3, 32, 32), save_path="./runs", post_compress_epochs=False):
    ...
    if data is not None and "accuracies" in data and "losses" in data:
...
def run_jf_experiment(experiments, model_path_097, train_loader, test_loader, device, epochs, pretrain, model_class=VGG16, model_kwargs=None, data_shape=None, save_path="./runs", post_compress_epochs=False):
    ...
    for exp_name, collapse_range in experiments.items():
        ...
        plot_results(
            jf_param_counts, jf_final_accuracies, jf_exp_names, "JF Experiment",
            os.path.join(plot_dir, "jf_experiment_results.svg"),
...
def run_kevin_experiment(experiments, model_path_000, train_loader, test_loader, device, epochs, model_class=VGG16, model_kwargs=None, data_shape=None, save_path="./runs", post_compress_epochs=False):
    ...
    for exp_name, collapse_range in experiments.items():
        ...
        os.makedirs(plot_dir, exist_ok=True)
        plot_results(
            kevin_param_counts, kevin_final_accuracies, kevin_exp_names, "Kevin Experiment",
            os.path.join(plot_dir, "kevin_experiment_results.svg"),
...
def run_nick_experiment(experiments, model_path_000, train_loader, test_loader, device, epochs, pretrain, model_class=VGG16, model_kwargs=None, data_shape=None, save_path="./runs", post_compress_epochs=False):
    ...
    for exp_name, collapse_range in experiments.items():
        ...
        plots_dir = os.path.join(save_path, "plots")
        os.makedirs(plots_dir, exist_ok=True)
        if data and "accuracies" in data and "losses" in data:
        ...
        # Plot results
        plot_results(
            nick_param_counts, nick_final_accuracies, nick_exp_names, "Nick Experiment",
            os.path.join(plot_dir, "nick_experiment_results.svg"),
