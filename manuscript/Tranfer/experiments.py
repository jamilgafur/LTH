...
def run_experiment(model, model_kwargs=None,
    ...
    ckpt_path = os.path.join(ckpt_dir, get_checkpoint_filename(workflow, exp_name, model.__class__.__name__, pretrain, epochs))
    ...
    return param_count, data["final_accuracy"], infer_time, mem_usage


def run_jf_experiment(experiments, model_path_097, train_loader, test_loader, device, epochs, pretrain,
                      model_class=VGG16, model_kwargs=None, data_shape=None,
                      save_path="./runs", post_compress_epochs=False):
...
def run_nick_experiment(experiments, model_path_000, train_loader, test_loader, device,
    ...
    for exp_name, collapse_range in experiments.items():
        ...
        # 1) Pretrain on original model weights
        print(f"Pretraining for {epochs + pretrain} epochs...")
        train_and_evaluate(base_model, train_loader, test_loader, device, epochs + pretrain, post_compress_epochs=False)

        # 2) Collapse AFTER pretraining on the trained model's weights (fixing the lost pre-compression dictionary issue)
        if collapse_range is not None:
            # Save current trained weights temporarily
        ...
        # 3) Finetune compressed model if requested
        param_count, final_acc, infer_time, mem_usage = run_experiment(
            model=base_model,
            model_kwargs=model_kwargs,
            train_loader=train_loader,
