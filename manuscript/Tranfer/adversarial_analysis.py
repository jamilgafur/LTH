...
# ---------------------------------------------------------------------------
# Utility: Merge parallel CSV summary files generated by concurrent runs.
# ---------------------------------------------------------------------------
def merge_parallel_csvs(output_dir: str, base_name: str = "summary") -> None:
    """Merge ``{base_name}_*.csv`` files into a single ``{base_name}.csv``.

    Parallel ``generate`` jobs write per‑model CSVs to avoid race conditions.
    Downstream phases expect a unified ``summary.csv``. This helper scans the
    ``output_dir`` for matching files, concatenates them (dropping exact
    duplicate rows), and writes the combined CSV back to ``output_dir``.
    """
    import glob
    import pandas as pd
...
# ---------------------------------------------------------------------------
# Helper: Load results from multiple experiment runs (different epochs).
# ---------------------------------------------------------------------------
def load_multiple_runs(root_dir: str) -> Tuple[List[Dict], List[Dict]]:
    """Collect summary and transfer records from all ``adversarial_results_*``
    directories under *root_dir*.

    The function adds a new field ``run`` to each record, derived from the
    directory name (e.g. ``ep100_pre300``). This enables the plotting functions
    to differentiate runs and produce combined figures.
    """
    summary_records: List[Dict] = []
    transfer_records: List[Dict] = []

    # Find all result folders matching the pattern used in the repository.
    ...
    for d in result_dirs:
        run_name = os.path.basename(os.path.normpath(d))  # e.g. ep100_pre300
        summary_path = os.path.join(d, "summary.csv")
        transfer_path = os.path.join(d, "transferability.csv")
        try:
            ...
            print(f"[ERROR] Failed to load CSVs from {d}: {e}")

    print(f"[INFO] Loaded {len(summary_records)} summary rows and {len(transfer_records)} transfer rows from {len(result_dirs)} runs")
    return summary_records, transfer_records
...
def generate_plots(output_dir: str, records: List[Dict], transfer_records: List[Dict]):
    ...
    # Plot 2: Transferability Heatmaps (now supports multiple runs)
    if transfer_records:
        ...
        # If the dataframe contains a ``run`` column we will generate a separate
        # set of heatmaps for each run, otherwise we fall back to the original
        # behaviour (single combined plot).
        ...
        for run in run_values:
            ...
            for dataset in tf_subset_run["dataset"].unique():
                for attack in tf_subset_run["source_attack"].unique():
                    tf_subset = tf_subset_run[(tf_subset_run["dataset"] == dataset) & (tf_subset_run["source_attack"] == attack)]
                    ...
                    if pivot_data is not None and not pivot_data.empty:
                        ...
                        plt.title(
                            f"Adversarial Transferability – {dataset} ({attack}){run_suffix}",
                            fontsize=14,
                            fontweight='bold',
                        )
                        ...
                        plt.tight_layout()
                        plt.savefig(
                            os.path.join(output_dir, f"transferability_heatmap_{dataset}_{attack}{run_suffix}.png"),
                            dpi=300,
                        )
                        plt.close()
                        print(f"[INFO] Saved: transferability_heatmap_{dataset}_{attack}{run_suffix}.png")

                    # Full model‑kind matrix.
                    ...
                    if full_pivot is not None and not full_pivot.empty:
                        full_pivot.to_csv(
                            os.path.join(output_dir, f"transferability_matrix_full_{dataset}_{attack}{run_suffix}.csv")
                        )
                        ...
                        plt.title(
                            f"Full Transferability Matrix – {dataset} ({attack}){run_suffix}",
                            fontsize=14,
                            fontweight='bold',
                        )
                        ...
                        plt.tight_layout()
                        plt.savefig(
                            os.path.join(output_dir, f"transferability_heatmap_full_{dataset}_{attack}{run_suffix}.png"),
                            dpi=300,
                        )
                        plt.close()
                        print(f"[INFO] Saved: transferability_heatmap_full_{dataset}_{attack}{run_suffix}.png")

                    # Pair‑type summary – keep a separate plot per run.
                    ...
                    if not pair_summary.empty:
                        ...
                        plt.title(
                            f"Normalized Transferability by Pair Type – {dataset} ({attack}){run_suffix}",
                            fontsize=14,
                            fontweight='bold',
                        )
                        ...
                        plt.tight_layout()
                        plt.savefig(
                            os.path.join(output_dir, f"transferability_pairtype_{dataset}_{attack}{run_suffix}.png"),
                            dpi=300,
                        )
                        plt.close()
                        print(f"[INFO] Saved: transferability_pairtype_{dataset}_{attack}{run_suffix}.png")
...
def main():
    ...
    if args.mode in ["full", "analyze"]:
        ...
        # Consolidate any per‑model CSV fragments generated by parallel runs.
        merge_parallel_csvs(args.output_dir)

        # If not in full mode, need to load the unified summary.
    ...
    if args.mode in ["full", "plot"]:
        ...
        if args.mode == "plot":
            # Ensure any fragmented CSVs are merged before plotting.
            merge_parallel_csvs(args.output_dir)
            summary_path = os.path.join(args.output_dir, "summary.csv")
