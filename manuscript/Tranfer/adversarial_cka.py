"""CKA feature similarity experiment utilities."""
        ...
        for (model_name, dataset_name, kind), model in model_cache.items():
            ...
            try:
                compression_set = CheckpointManager.get_compression_set_for_checkpoint(model_name, base_dataset, ckpt_path)
            except Exception:
                continue
            ...
            for block in compression_set:
                ...
                if not start or not end:
                    continue
                try:
                    x_in, y_out, _ = _capture_preblock_activation(model, start, end, input_shape, [], None, device, debug=False)
    ...
    def _plot_cka_from_dataframe(cls, output_dir: str, df: pd.DataFrame) -> None:
        ...
        for dataset_name in df["dataset"].unique():
            ...
            fig.suptitle(f"Figure 5: CKA Similarity by Model and Variant - {dataset_name}", fontsize=15, fontweight="bold")
            ...
            for idx, model_name in enumerate(model_names):
                ...
                summary = model_df.groupby(["source_kind", "target_kind"], as_index=False)["cka"].mean().pivot(
                    index="source_kind", columns="target_kind", values="cka"
                )
                summary = summary.reindex(index=variant_order, columns=variant_order)
                sns.heatmap(summary, annot=True, fmt=".3f", cmap="YlOrRd", vmin=0, vmax=1, cbar_kws={"label": "Mean CKA"}, ax=ax)
