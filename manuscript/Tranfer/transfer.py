...
InceptionNet_common = {
    ...
    # --- 3. Granular Probes: The "Danger" Zones (High Variance Spikes) ---
    "Stage 5a Only": ("stage5.inception_5a", "stage5.inception_5a"),
    "Stage 5b Only": ("stage5.inception_5b", "stage5.inception_5b"),

    # --- 4. Multi-Stage Combinations ---
    "Stages 2 and 3": ("stage2.conv2", "stage3.inception_3b"),
    "Stages 3 and 4": ("stage3.inception_3a", "stage4.inception_4e"),
}
...
def run_experiments_for_dataset(
    ...
    # ------------------------------------------------------------------------
    # STEP 1.5: Generate Journal Visuals (Heuristics & Architecture)
    # ------------------------------------------------------------------------
    print("\n[STEP] Generating Heuristic and Architectural Visuals...")
    
    # Locate the CSV generated by the profiling step
    csv_path = os.path.join("runs", "Layer_Statistics", f"{args.model}_{dataset}_layer_stats.csv")
    ...
    if os.path.exists(csv_path):
        ...
        print(f"[WARN] Could not find {csv_path} to generate heuristic plots.")
