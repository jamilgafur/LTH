    ...
    for (dataset, arch), g_metrics in df.groupby(["dataset", "architecture"]):
        ...
        for is_quant_target in [False, True]:
            ...
            for exp_name, ranges in model_exps.items():
                ...
                # [CRITICAL FIX] Use global clean_exp_name to ensure "Control_Continuted" maps
                # perfectly to "Control (Continued)" just like the dataframe initialization.
            ...
            # --- POPULATE CSV DATA ---
            for orig_name, r_ranges, res, d_name, p_r, f_r, m_r in valid_exps:
                fig4_csv_data.append({
                    "Architecture": arch,
                    "Dataset": format_dataset_name(dataset),
                    "Experiment": d_name,
                    "Is_Quantized": is_quant_target,
                    "Collapsed_Ranges": str(r_ranges),
                    "Delta_Acc": res.get('d_acc', np.nan),
                    "Params_Reduction_%": p_r,
                    "FLOPs_Reduction_%": f_r,
                    "Memory_Reduction_%": m_r
                })
        ...
        if fig4_csv_data:
            csv_path = out_dir / f"{arch}_{dataset}_search_space_metrics.csv"
            pd.DataFrame(fig4_csv_data).to_csv(csv_path, index=False)
            logger.info(f"[FIG4] Saved search space data to CSV at {csv_path}")
